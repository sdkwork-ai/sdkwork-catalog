...
All L2+ `app-api`, `backend-api`, and SDKWork-owned `open-api` success JSON bodies `MUST` use `SdkWorkApiResponse` from `API_SPEC.md` §15:

- Envelope: `{ "code": 0, "data": <payload>, "traceId": "<server-uuid>" }`
- Success `code` is numeric `int32`; HTTP 2xx JSON bodies `MUST` use `0` only. REST semantics remain on HTTP status (`201`, `202`, etc.).
- Platform error codes are numeric non-zero values per §15.3 (`40001`, `40101`, `40401`, …).
- Single resource: `data.item`
- Lists: `data.items` + `data.pageInfo` (`PageInfo.mode` is `offset` or `cursor`)
- Commands: `data.accepted` plus optional `resourceId` / `status`
- Async accept (`202`): `data.operationId`, `data.status`, optional `pollUrl`
...
Authority: `sdkwork-specs/API_SPEC.md` §15–§16, `SDK_SPEC.md` §4.2, `FRONTEND_SPEC.md`, `MIGRATION_SPEC.md` §4.2.
