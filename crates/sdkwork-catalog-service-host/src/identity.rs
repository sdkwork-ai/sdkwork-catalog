//! Snowflake identity bootstrap for the Catalog composition host.
//!
//! Catalog tables use platform Snowflake `BIGINT` primary keys (`DATABASE_SPEC.md` §6.1,
...
//!
//! Catalog writes through the **merchandise** catalog repository, which mints both its own table
//! keys and the ids the HTTP surface publishes. That store takes this same generator as a
//! constructor argument, so one process never runs two sequences over one node id.
//!
...
use crate::runtime_env::{catalog_environment_name, catalog_is_production_like};

/// Static node id escape hatch, honoured only outside production-like environments.
pub const CATALOG_SNOWFLAKE_NODE_ID_ENV: &str = "SDKWORK_CATALOG_SNOWFLAKE_NODE_ID";

/// Logical service name registered in the platform node registry.
pub const CATALOG_IDENTITY_SERVICE_NAME: &str = "sdkwork-catalog";
...
/// `SDKWORK_DATABASE_*`.
pub const CATALOG_IDENTITY_DATABASE_SERVICE: &str = "CATALOG";
...
///
/// Split out from [`CatalogIdentity::from_pool`] so the fail-closed rule is testable without an
...
pub fn identity_posture(
    ...
    match configured_node_id {
        Some(value) => Err(format!(
            "static {CATALOG_SNOWFLAKE_NODE_ID_ENV}={value} is forbidden in production-like \
...
/// The Catalog process identity: one Snowflake generator plus the lease that keeps its node id
/// reserved.
///
...
impl CatalogIdentity {
    ...
    /// the same authority as the Catalog tables, and reusing the pool keeps the lease alive for the
    ...
    pub async fn from_pool(pool: &DatabasePool) -> Result<Self, String> {
        let posture = identity_posture(
            catalog_is_production_like(),
            std::env::var(CATALOG_SNOWFLAKE_NODE_ID_ENV).ok().as_deref(),
        )?;
    ...
    async fn leased_identity(pool: &DatabasePool) -> Result<Self, String> {
        let config = NodeAllocatorConfig::from_service_name(CATALOG_IDENTITY_SERVICE_NAME);
        let (generator, lease) = SnowflakeNodeAllocator::allocate_process_generator(pool, &config)
            .await
            .map_err(|error| format!("allocate Catalog Snowflake node lease failed: {error}"))?;

        tracing::info!(
            node_id = lease.node_id(),
            service = CATALOG_IDENTITY_SERVICE_NAME,
            database_service = CATALOG_IDENTITY_DATABASE_SERVICE,
            "catalog acquired Snowflake node lease"
        );
    ...
    pub fn development_identity() -> Result<Self, String> {
        let node_id = match std::env::var(CATALOG_SNOWFLAKE_NODE_ID_ENV) {
            Ok(value) => value
                .trim()
                .parse::<u16>()
                .map_err(|error| format!("invalid {CATALOG_SNOWFLAKE_NODE_ID_ENV}: {error}"))?,
        ...
        tracing::warn!(
            node_id,
            environment = catalog_environment_name(),
            "catalog using a static Snowflake node id; this is only valid outside production-like \
             environments"
        );
...
impl IdGenerator for CatalogIdentity {
...
#[must_use]
pub fn shared_identity(identity: CatalogIdentity) -> Arc<CatalogIdentity> {
    Arc::new(identity)
}
...
mod tests {
    ...
    fn production_refuses_a_static_node_id() {
        ...
        assert!(
            error.contains(CATALOG_SNOWFLAKE_NODE_ID_ENV),
    ...
    fn the_injected_port_produces_a_decimal_bigint_string() {
        let identity = CatalogIdentity::from_generator(
